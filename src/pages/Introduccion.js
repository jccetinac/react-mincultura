import React from 'react';

export default function Introduccion() {
  return (
    <div className="page card ">

      <h1 className="titulo">INTRODUCCIÓN</h1>

      <p className="parrafo">
      El documento que los lectores tienen en su mano es, además de un lineamiento pedagógico, un llamado a reconocer el aporte que han hecho los pueblos y comunidades afrocolombianas, negras, populares y campesinas a la música y la cultura del país. Representa además el interés del Plan Nacional de Música para la Convivencia por resaltar el valor de estas músicas en la formación musical del país y la persistencia de las comunidades en cuidarlas, recrearlas y asegurar su permanencia en el tiempo. 
      </p>

      <p className="parrafo">
      Por muchos años, la formación musical formal y no formal ha privilegiado los referentes pedagógicos de otros países, más que las pedagogías propias de las músicas de tradición oral, es decir, aquellas que se enseñan, aprenden y reproducen a través de la oralidad, del gesto, de la imitación y no desde la escritura. Con ello, en muchas regiones con una rica tradición musical y cultural, se ha dado la espalda a las propias formas de enseñar, aprender y apropiar las músicas, para adoptar metodologías ajenas que se presentan como “más valiosas y eficientes”. 
      </p>

      <p className="parrafo">
      En la academia, son escasas las investigaciones sobre la <b>diversidad de pedagogías de las músicas tradicionales, populares y regionales,</b> manteniéndose así la primacía de la mirada centroeuropea sobre la formación musical. La investigación se ha centrado en la sistematización de repertorios y técnicas (Rojas, 2004) (Ochoa F. , 2013). Se trata de un trabajo que logra material pedagógico valioso, aunque no profundiza en los enfoques particulares de aprendizaje. Un número limitado de estudios analiza el aprendizaje de los instrumentos tradicionales en contextos informales a la vez que indaga sobre su impacto en los entornos formales (Miñana, 1987) (Convers & Ochoa, 2007). En el caso de las cuerdas pulsadas, la investigación se centra principalmente en el análisis musicológico de los estilos de los músicos tradicionales colombianos (Lambuley, 2008) y la sistematización de técnicas, sistemas rítmicos y armónicos (Sossa J. , 1988) mientras que los estudios sobre literatura informal recogen métodos y cancioneros (Paredes, 2006) (Borrero, s.f). 
      </p>

      <p className="parrafo">
      De esta manera, las músicas de tradición oral han sido materia de investigaciones musicales más que pedagógicas. Han sido fuente de innovación y creación para otras músicas a través de intervenciones como las fusiones, por ejemplo, y han sido apropiadas por la academia y por músicos ajenos a las poblaciones y a los territorios donde se producen, para ser incluidas en los entornos musicales; es el caso de las ciudades donde se adelantan propuestas categorizadas como músicas urbanas. Con esto, en la mayoría de los casos a pesar de que las músicas de tradición oral han sido valoradas y recreadas, también han sido alejadas de sus lógicas y contextos de producción y aprendizaje, perdiendo así una parte considerable de su esencia. 
      </p>

      <p className="parrafo">
      En este sentido, puede decirse que el país está en mora de reconocer las pedagogías de las músicas de tradición oral como una fuente de enriquecimiento para otras músicas y pedagogías. Este documento intenta avanzar en la resolución de ese vacío aportando <b>una base inicial de conocimientos sobre las pedagogías de las músicas de tradición oral, </b>
      que permite continuar estructurando los saberes, para conocer y ampliar las diversas maneras de apropiación de estas músicas en los distintos contextos, poblaciones y territorios. 
      </p>

      <p className="parrafo">
      Desde 1993, Plan Nacional de Música para la Convivencia, se ha propuesto recoger en diferentes cartillas, orientaciones pedagógicas para el abordaje de las músicas tradicionales. Fue así como se llegó a la organización de 11 ejes (ver Cuadro 1) y a la configuración de cartillas correspondientes a cada uno.
      </p>
      
      <h6>Cuadro 1. Ejes de músicas tradicionales y géneros por territorio</h6>

      <table className="tablamin">
       
      
       <tr>
          <th>Eje</th>
          <th>Géneros</th>
          <th>Territorios</th> 
          </tr>

          <tr className="par">
            <td>Músicas isleñas </td>
            <td>Calypso, schottis, soka y otros</td>
            <td>San Andrés y Providencia.</td>
          </tr>  

          <tr className="impar">
            <td>Músicas de acordeón y cuerdas </td>
            <td>Formatos de acordeón y guitarras con músicas de son, merengue, puya y paseo </td>
            <td>Guajira, Cesar y Magdalena.</td>
          </tr>  

          <tr className="par">
            <td>Músicas de pitos y tambores  </td>
            <td>Gaitas largas y corta, millo, baile cantao, tambora y bandas tipo pelayera que interpretan puya, bullerengue, porro, fandango y otros. </td>
            <td>Atlántico, Bolívar, Sucre y Córdoba.</td>
          </tr>  

          <tr className="impar">
            <td>Chirimías y cantos tradicionales  </td>
            <td>Porro chocoano, abozao, alabaos y otros cantos tradicionales.  </td>
            <td>Chocó.</td>
          </tr>

          <tr className="par">
            <td>Músicas de marimba y cantos tradicionales  </td>
            <td>Currulao, bunde, berejú, arrullos y otros.  </td>
            <td>Litoral Pacífico del Valle, Cauca y Nariño.</td>
          </tr>

          <tr className="impar">
            <td>Músicas andinas sur-occidente </td>
            <td>Formatos campesinos, bandas de flautas, andino sureño con músicas como sanjuanito, pasillo, tinku, y otros.  </td>
            <td>Cauca, Nariño y occidente del Putumayo.</td>
          </tr>


          <tr className="par">
            <td>Músicas andinas centro-sur </td>
            <td>Formato cucamba, tríos y duetos vocales que interpretan rajaleña, caña, san juanero, bambuco y otros. </td>
            <td>Huila y Tolima.</td>
          </tr>

          <tr className="impar">
            <td>Músicas andinas centro-oriente  </td>
            <td>Formato de torbellino, estudiantinas, tríos y prácticas vocales con músicas como rumba, carranga, bambuco, guabina y otros.  </td>
            <td>Norte de Santander, Santander, Boyacá́ y Cundinamarca.</td>
          </tr>

          <tr className="par">
            <td>Músicas andinas centro-occidente </td>
            <td>Formatos campesinos, estudiantinas y prácticas vocales que interpretan pasillo, bambuco, shotis y otros. </td>
            <td>Valle, Antioquia, Quindío, Risaralda y Caldas.</td>
          </tr>

          <tr className="impar">
            <td>Músicas llaneras </td>
            <td>Formato de arpa y bandola llanera con cuatro y capacho que interpretan joropo.</td>
            <td>Vichada, Arauca, Guaviare, Meta, Casanare y oriente de Cundinamarca y Boyacá.</td>
          </tr>

          <tr className="par">
            <td>Músicas de la Amazonía  </td>
            <td>Formatos diversos con músicas de frontera que interpretan músicas de Colombia y otros países.</td>
            <td>Caquetá, Amazonas, Putumayo, Guaviare, Vaupés y Guainía.5 </td>
          </tr>
          
      </table>

     <h6>Tomado de: (Arango, 2009, pág. 244)</h6>
      <br>
      </br>

      <p className="parrafo">
      Estas cartillas definieron rutas de abordaje a través de niveles<sup><a data-toggle="modal" data-target="#notapie1">1</a></sup> : ritmo percusivo, ritmo armónico, ritmo melódico e improvisatorio, enfocándose en algunas formas de estructuración de estas músicas, pero no en las características de las pedagogías de las mismas. En años posteriores, desde el área de música del Ministerio de Cultura, se han diseñado lineamientos de formación musical en los que se han recogido los aportes pedagógicos de diversas corrientes y procedencias. De esta manera, las cartillas y lineamientos anteriores, se han centrado en las músicas y sus formatos y han propuesto caminos para su abordaje.
      </p>
      <p className="parrafo">
         <span className="destacado-especial">    
            A diferencia de los anteriores documentos, este lineamiento tiene como centro las pedagogías de las músicas de tradición oral; aunque encuentra asidero en las músicas mismas, <b>su énfasis son sus pedagogías</b>, es decir los modos en los que se apropian e intervienen las mismas, las formas de construcción de conocimiento musical. 
         </span>
      </p>
      
      <p className="parrafo">
        Este lineamiento de Pedagogías de las músicas de tradición oral, guarda algunas conexiones con los trabajos anteriores:
      </p>

      <ul>
        <li>
      	Tiene como base las lógicas de las músicas tradicionales organizadas en ejes, como se señala en el cuadro 1, así como algunos de los repertorios propuestos en las cartillas de músicas tradicionales. 
        </li>
        <li>
      	Recoge algunas de las elaboraciones de los lineamientos de formación musical, como algunos ejes formativos, sentidos y pautas para los procesos pedagógicos. Puede relacionarse por ejemplo el eje auditivo de los lineamientos anteriores, con la escucha, propuesta en el presente documento. La exploración, la creación y la práctica están presentes en todos los lineamientos, incluido este.
        </li>
      </ul>

      <p className="parrafo">
        Debe señalarse que, en este lineamiento, los elementos señalados anteriormente se proponen desde la lógica de las músicas de tradición oral, por lo que se alerta al lector o lectora, que tendrán un sentido particular en este documento. Se busca así evidenciar la diversidad de propuestas de abordaje de la formación musical, manteniendo algunos elementos que se consideran fundamentales.
      </p>

      <p className="parrafo">
       A pesar de que este lineamiento no es un documento de investigación sobre las pedagogías de las músicas de tradición oral, sí se propone como una entrada, como una provocación a su exploración y valoración, como acciones que se constituyen en parte fundamental de las músicas mismas. Por ello, para su realización, se partió de un proceso de identificación de diferentes formas de apropiación de las músicas de tradición oral de algunas regiones del país (capítulo 1), para posteriormente construir una propuesta pedagógica general para las Escuelas Municipales de Música. De esta manera, el lineamiento recoge los aportes de las pedagogías de las músicas de tradición oral, sus lógicas y contextos, con el fin de proponer pautas para la formación y la práctica musical de estas Escuelas y de aquellos centros de formación musical a los cuales pueda resultarles atractivo e interesante (capítulo 2).
      </p>

      <p className="parrafo">
      En este sentido, el presente lineamiento <b>no busca orientar o reemplazar los procesos pedagógicos de las músicas tradicionales, sino nutrir otras prácticas pedagógicas</b> recurriendo a los saberes acumulados alrededor de estas músicas. Con este propósito, el lineamiento describe:
      </p>

      <ul>
        <li>
      	Dimensiones: La Oralidad, la Memoria y la Corporalidad hacen parte de los procesos de aprendizaje, apropiación y enseñanza de las músicas de tradición oral y se consideran un aporte fundamental a las pedagogías musicales.
        </li>
        <li>
      	Procesos transversales: La Observación, la Escucha, la Imitación y la Exploración, son entendidos como procesos presentes en todo el proceso formativo que permiten a los docentes y estudiantes acercarse a las músicas de tradición oral desde las vías propias de las comunidades que practican y viven estas músicas. 
        </li>
        <li>
        Líneas pedagógicas: integran las dimensiones y procesos básicos, sugiriendo un camino para el abordaje de estas músicas. 
        </li>
        <li>
        Un anexo con repertorios musicales y de la tradición oral que sugieren algunos niveles de complejidad para el proceso formativo. 
        </li>
      </ul>

      <p className="parrafo">
      Finalmente, este lineamiento propone un camino para que, a través de la investigación en Escuelas Municipales de Música, se hagan acercamientos a las formas de aprendizaje, enseñanza, apropiación y recreación de las músicas de tradición oral de las diferentes regiones del país, que permitan nutrir la labor pedagógica desde la diversidad cultural y musical con la que contamos (capítulo 3). 
      </p>

      <p className="parrafo">
        Las <b>orientaciones pedagógicas</b> aquí contenidas no son métodos o pasos que deban seguirse de manera estricta, sino pautas generales que pueden ser adaptadas a unas músicas específicas y a unos contextos particulares. Así, los docentes de las Escuelas Municipales de Música podrán contar con unas orientaciones de una forma de entender, descubrir y enseñar, que podrán aterrizar a unas músicas y una población en particular. 
      </p>

      <p className="parrafo">
      En conjunto con este documento se espera resaltar el valor que tienen las pedagogías de las músicas de tradición oral en el desarrollo de la memoria, la capacidad creativa y el disfrute. En consonancia con María Eugenia Londoño, se entiende que la recuperación de las memorias musicales colectivas es parte fundamental de la reconstrucción del tejido social que ha sido deteriorado por la guerra en el país: 
      </p>
     <em  className="confondo">
        Colombia, violentada y desgarrada por la inequidad y la guerra, reclama una recuperación crítica de la memoria musical colectiva, capaz de orientar y fortalecer la acción oficial en diversos escenarios, memoria recreadora orientada a redireccionar las prácticas académicas reconocidas e institucionalizadas durante décadas. (Londoño, 2009, pág. 58)
     </em>

     <p className="parrafo">
     Se espera también dar impulso al diálogo entre las músicas académicas y las músicas de tradición oral, reconociendo y sistematizando algunos de los elementos fundamentales de esta última. Finalmente, espera que los docentes de las Escuelas Municipales de Música tengan un insumo que les permita diversificar y enriquecer su quehacer pedagógico y contribuyan al fortalecimiento de las músicas de tradición oral del país. 
    </p>

    <p className="parrafo">
    La construcción de este documento fue posible gracias al trabajo conjunto de representantes de algunas de las músicas de tradición oral, así como de personas cuyas experiencias los han llevado a escudriñar a profundidad el valor pedagógico arraigado en diferentes comunidades del país. La conversación y la reflexión desde el hacer, establecieron la ruta a través de la cual este documento tomó forma. De esta manera, el presente lineamiento se hizo posible gracias al afecto, dedicación e interés de:
    </p>

     <p className="parrafo">
       <span className="destacado-especial">Ana Iris Castillo, cantadora tradicional, codirectora del Grupo Tizón de la ciudad de Pasto. </span>Ganadora de la mejor canción inédita en el Festival de Música Petronio Álvarez 2018. Directora de la fundación Folclórica de Música Tumbos de Cualimán. Miembro del Grupo Gestor Regional del Plan Especial de Salvaguardia. Licenciada en Básica Primaria con énfasis Lengua Castellana y especialista en Pedagogía.
     </p>

    <p className="parrafo">
     <span className="destacado-especial">Cesar de Jesús Hernández Músico tradicional y cultor de los Montes de María. Músico tradicional de conjuntos gaiteros. </span>Ex director de la Casa de la Cultura. Investigador sobre los focos de música de gaitas en los Montes de María. Fundador del grupo Aires de los Montes de María en 1992. Maestro del proyecto Semilleros de Iniciación Musical de la Escuela de Música Lucho Bermúdez. Licenciado en educación básica con énfasis en lengua castellana e inglés. Especialista en Pedagogía. Técnico Laboral en Música Instrumental de Bellas Artes y Humanidades.
    </p>

    <p className="parrafo">
       <span className="destacado-especial">Efraín Franco: Integrante del grupo Canto al pueblo-Armadillo (1977-1984) dedicado a la recopilación, estudio e interpretación de músicas tradicionales. </span>Autor del Documento base de cartografía de músicas tradicionales del Eje Centro Oriente (2004) y coautor de “Viva quien toca” Cartilla de Iniciación Musical, Músicas Andinas del Centro-Oriente Colombiano (2008) con el Ministerio de Cultura. Director del Ensamble de Músicas Campesinas de la ASAB-Universidad Distrital Francisco José de Caldas desde 2008.  
    </p>

    <p className="parrafo">
      <span className="destacado-especial">Gustavo Córdoba –Hijuelapo- es pintor, músico folclórico y gestor cultural. </span>Nació en el municipio de Campoalegre – Huila, fue director de la Casa de la Cultura de Campoalegre y Consejero departamental de cultura en los años 2.000 - 2004. Con coplas y canciones de su autoría, condensa el sentir del pueblo Campoalegruno y Huilense y promueve un movimiento de la música folclórica que representa las luchas y la visión del pueblo Huilense, que defiende su territorio, sus tradiciones culturales, la paz y la convivencia.
    </p>

    <p className="parrafo">
      <span className="destacado-especial">Jhon Moreno: Nació en San Luís de Palenque, Casanare. Músico de la Universidad de los Andes. </span>Entre otras labores, ha acompañado al Ministerio de Cultura en la formación de investigadores en música llanera para el eje del “Territorio Sonoro del Joropo” y ha coordinado el Plan Especial de Salvaguardia de Carácter Urgente de Cantos de Trabajo de Llano y el Plan Departamental de Música del Meta.
    </p>

    <p className="parrafo"> 
      <span className="destacado-especial"> Jorge Sossa: Músico, docente e investigador de las músicas populares y tradicionales. Director de la Escuela Musical Nueva Cultura. </span> Miembro del Comité permanente del Movimiento de la Canción Infantil Latinoamericana y caribeña, MOCILyC Coordinador de la Red Colombiana de la Canción Infantil, RedCi, Realizador del programa radial Con alma de niños de UNRadio, emisora de la Universidad Nacional de Colombia.
    </p>

    <p className="parrafo">
    Esperamos sea de interés y utilidad, pero, sobre todo, que permita que, desde la música, cultivemos el orgullo de las tradiciones que se han resistido al olvido y hacen parte esencial de lo que somos como colombianos.
    </p>  
    
    
<div id="notapie1" class="modal fade" role="dialog">
  <div class="modal-dialog">

    
    <div class="modal-content ">
      <div class="modal-header"> <button type="button" class="closemodal" data-dismiss="modal">&times;</button><h4 class="modal-title numeromodal">1</h4> 
      </div>
      <div class="modal-body">
        <p>
        Si bien en las cartillas de músicas tradicionales se propuso el concepto de “niveles”, en este lineamiento se hace alusión a “ámbitos de organización de lo sonoro”, pues se considera que la idea de niveles remite a una jerarquía. Con ámbitos hacemos alusión a una organización sistémica y no jerárquica de la organización de lo sonoro en las músicas de tradición oral. 
         </p>
      </div>

      </div>
      </div>
      </div>



    </div>
  );
}


/*MODALES*/
